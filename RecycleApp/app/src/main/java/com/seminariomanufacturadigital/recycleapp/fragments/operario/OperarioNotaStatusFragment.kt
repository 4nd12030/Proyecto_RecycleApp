...
class OperarioNotaStatusFragment : Fragment() {

    var  myView: View? = null
    var TAG = "OperarioNotaStatusFragment"
    var recyclerViewNotas: RecyclerView? = null
    var adapter: NotasOperarioAdapters? = null
    ...
    private fun getNotas() {
        ...
        notaProvider?.getNotasByEmpleadoAndStatus(usuario?.numEmpleado!!, status)?.enqueue(object: Callback<ArrayList<Nota>> {
            override fun onResponse(
                ...
                if(response.body() != null){
                    ...
                    Log.d(TAG, "${notas}")
                    adapter = NotasOperarioAdapters(requireActivity(), notas!!)
