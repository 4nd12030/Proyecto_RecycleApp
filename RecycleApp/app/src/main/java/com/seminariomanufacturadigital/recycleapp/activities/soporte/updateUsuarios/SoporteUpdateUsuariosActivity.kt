package com.seminariomanufacturadigital.recycleapp.activities.soporte.updateUsuarios
...
class SoporteUpdateUsuariosActivity : AppCompatActivity() {
    ...
    var TAG = "SoporteUpdateUsuariosActivity"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        usuario = intent.getStringExtra("empleadoEditado")
        Log.d(TAG, "Usuario a editar: ${usuario}")
        empleado =  Gson().fromJson(usuario, Empleado::class.java)
        ...
        empleadosProvider = EmpleadosProviders(empleadoActual?.sessionToken)
        Log.d(TAG, "Session Token Empleado ${empleadoActual?.sessionToken}")
...
/////////////////////////////////////////////////////////////////
    ...
    private fun updateData() {
       ...
       var telefono = editTextTelefono?.text.toString()
        ...
        Log.d(TAG, "Empleado nuevo: ${empleado} ")
        if(imageFile != null){
            empleadosProvider?.update(imageFile!!, empleado!!)?.enqueue(object : Callback<ResponseHttp> {
                override fun onResponse(
                    ...
                    Toast.makeText(this@SoporteUpdateUsuariosActivity, response.body()?.message, Toast.LENGTH_LONG).show()
                ...
                override fun onFailure(call: Call<ResponseHttp>, t: Throwable) {
                    ...
                    Toast.makeText(this@SoporteUpdateUsuariosActivity, "Error ${t.message}", Toast.LENGTH_LONG).show()
                }
            })
        } else {
    ...
    private fun getEmpleadoFromSesion() {
        if (!sharedPref?.getData("empleado").isNullOrBlank()) {
            ...
            Log.d(TAG, " Empleado actual en fun getEmpleadoFromSesion: $empleadoActual")
        }
    }

//    private fun saveEmpleadoInSesion(data: String){
//        val empleadoActual = gson.fromJson(data, Empleado::class.java)
//        sharedPref?.save("empleado", empleadoActual)
//        Log.d(TAG, "Empleado guardado en sesion: $empleadoActual")
//    }

//    private fun getEmpleadoParaEditar() {
//        if (!sharedPref?.getData("empleadoEditado").isNullOrBlank()) {
//            //Si el usuario existe en sesion
//            empleadoAEditar = gson.fromJson(sharedPref?.getData("empleadoEditado"), Empleado::class.java)
//            Log.d(TAG, " Empleado en fun getEmpleadoParaEditar: $empleadoActual")
//        }
    }
